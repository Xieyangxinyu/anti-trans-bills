...
```{r monthly trend overall}
bills_18_23 <- bills_18_23[(bills_18_23$year >= 2018), ]
bills_18_23 <- bills_18_23[!is.na(bills_18_23$year), ]

library(lubridate)

trend <- bills_18_23 %>% mutate(
    Date = ymd(Date),
    Month_Yr = ym(format_ISO8601(Date, precision = "ym"))
  )
...
sorted.data <- trend[order(trend$Month_Yr),]

data.length <- length(sorted.data$Month_Yr)
time.min <- sorted.data$Month_Yr[1]
time.max <- sorted.data$Month_Yr[data.length]
...
ix <- seq(1, length(merged.data$count), by=12)
fmt <- "%Y-%m" # format for axis labels
labs <- format(as.Date(merged.data$Month_Yr[ix]), fmt)

plot(merged.data$count, type = "l", lty = 1, xaxt ="n", main = "Number of Total Bills by Month", xlab = "", ylab = "Count")
axis(side = 1, at = ix, labels = labs)
```
...
care_type_summary <- care_type_mutate %>% 
  mutate(pass_num = (pass == "Passed")) %>%
  group_by(year, `Bill Type`) %>%
  summarise(count = sum(pass_num), percent = sum(pass_num)/length(pass_num))

care_type_summary %>% ggplot(aes(x=year, y=count, group=`Bill Type`)) +
            geom_line(aes(colour=`Bill Type`)) +
            geom_smooth(aes(group=`Bill Type`))

care_type_summary %>% ggplot(aes(x=year, y=percent, group=`Bill Type`)) +
            geom_line(aes(colour=`Bill Type`)) +
            geom_smooth(aes(group=`Bill Type`))
```
