...
def get_topic_model(bill_type: str = 'Healthcare'):
    ...
    if os.path.exists(f'data/{bill_type}'):
        ...
        df = pd.read_csv('data/bills_18_23.csv')
        ...
        umap_model = UMAP(n_neighbors=min(15, int(len(docs)/5)), n_components=10, min_dist=0.0, metric='cosine', random_state=42)
        ...
        client = openai.OpenAI(api_key="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")
        ...
        topic: <topic label>
        """
        openai_model = OpenAI(client, model="gpt-3.5-turbo-1106", exponential_backoff=True, chat=True, prompt=prompt)
